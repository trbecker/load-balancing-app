...
def auto_connect_work(connection_manager, ues, antennae):
    connection_manager.auto_connect(ues, antennae)

if __name__ == '__main__':
    import sys, getopt, connections
    ...
    simulation, antennae, ues = prepare_simulation(configfile, random_position)
    connection_manager = connections.ConnectionManager(simulation)
    rsrp, snr, rsrq, bler, cqi = simulation._simulate()
    auto_connect_thread =  Thread(target=auto_connect_work, args=(connection_manager, ues, antennae))
...
#            try:
#                admission_req = e2sim_client.UEIMSIAdmissionPutRequest(ue=ue_descr,
#                                nodeb=connection_bbu_id)
